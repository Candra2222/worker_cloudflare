export default {
  async fetch(request, env, ctx) {
    try {
      ...
      console.log('Access:', hostname, url.pathname); // Debug log
...
async function handleRedirect(req, env, sub, ctx) {
  try {
    ...
    // Deteksi Facebook Crawler (lebih lengkap)
    const isFB = ua.includes('facebook') || 
                 ua.includes('Facebot') || 
...
<head>
  ...
  <style>
    ...
    h2{color:#1d2129;margin:0 0 10px}
    p{color:#606770;margin:0 0 15px}
